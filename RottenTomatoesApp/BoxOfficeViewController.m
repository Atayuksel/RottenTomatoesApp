...
{
    NSURL *url = [NSURL URLWithString:@"http://api.rottentomatoes.com/api/public/v1.0/lists/movies/box_office.json?apikey=n594qzwyec5cdgr3tdrpfee3&limit=50"];
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        self.BoxOfficeMovies = [responseObject objectForKey:@"movies"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    tempDictionary= [self.BoxOfficeMovies objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.filmDetailViewController.filmDetail = [self.BoxOfficeMovies objectAtIndex:indexPath.row];
